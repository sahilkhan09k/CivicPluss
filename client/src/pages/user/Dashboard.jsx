        ...
        <div className="flex min-h-screen bg-gradient-to-br from-gray-50 via-primary-50/30 to-accent-50/20">
            ...
            <div className="flex-1 md:ml-64 p-4 pt-16 md:pt-4 md:p-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-8">
                    {/* Trust Score Widget */}
                    <Link to="/profile" className="stat-card hover:shadow-xl transition-shadow cursor-pointer col-span-1">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-gray-600 text-xs md:text-sm mb-1 font-medium">Trust Score</p>
                                <p className={`text-3xl md:text-4xl font-bold bg-gradient-to-r ${(user?.trustScore ?? 100) >= 75 ? 'from-green-600 to-green-700' :
                            ...
                            <div className={`p-2 md:p-3 rounded-xl ${(user?.trustScore ?? 100) >= 75 ? 'bg-green-100' :
                                    (user?.trustScore ?? 100) >= 50 ? 'bg-yellow-100' :
                                        (user?.trustScore ?? 100) >= 25 ? 'bg-orange-100' :
                                            'bg-red-100'
                                }`}>
                                <Shield className={`h-6 w-6 md:h-8 md:w-8 ${(user?.trustScore ?? 100) >= 75 ? 'text-green-600' :
                    ...
                    <div className="stat-card">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-gray-600 text-xs md:text-sm mb-1 font-medium">Reported</p>
                            ...
                            <div className="bg-primary-100 p-2 md:p-3 rounded-xl">
                                <AlertCircle className="h-6 w-6 md:h-8 md:w-8 text-primary-600" />
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 text-sm">
                                                <span className={`px-2 py-1 rounded-full font-medium text-xs ${getStatusColor(issue.status)}`}>
                                                    {issue.status}
                                                </span>
                                                <span className={`px-2 py-1 rounded-full font-medium text-xs ${getPriorityColor(issue.priority)}`}>
                                                    {issue.priority}
                                                </span>
                                        ...
                                        {issue.imageUrl && (
                                            <img
                                                src={issue.imageUrl}
                                                alt={issue.title}
                                                className="w-16 h-16 md:w-24 md:h-24 object-cover rounded-lg ml-3 flex-shrink-0"
                                            />
                                        )}
                                    </div>
                                </Link>
